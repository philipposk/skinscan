...
Signed-in pages carry the [page-assistant](https://github.com/philipposk/page-assistant)
widget, vendored as a submodule under `vendor/`. It is configured as narrowly as
the rest of the app:

- **It cannot diagnose, and there is nothing it could call if it tried.** No
  capability interprets a lesion; the persona refuses outright, including when
  pushed or when the user claims to be a clinician. Mention bleeding, a
  non-healing sore, fast growth or a nail stripe and it stops helping with the
  app and tells you to see a doctor.
- **It never receives a photograph.** `autoScan` is off, so it does not read the
  page you are on — which on these pages would mean every lesion label and
  outcome on screen. `/api/pa/context` is its only window onto your data:
  labels, sites, dates, counts, outcome band, case status. No images, no notes,
  no model rationales.
...
`/api/pa/llm.txt` is hand-written rather than generated by the SDK: the
generator advertises a POST endpoint that lets external agents drive the
assistant, and an app holding Article 9 health data should not have one.
